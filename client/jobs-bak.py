...
class Job(dict):
    ...
    def required_gpu_num(self, ):
        return self.__getitem__('num_gpus')
    
    @property
    def uesr_specified_deadline_time(self, ):
        if self.__getitem__('ddl_time') is not None:
            return self.__getitem__('ddl_time')
    
    @uesr_specified_deadline_time.setter
    def uesr_specified_deadline_time(self, value):
        if self.__getitem__('ddl_time') is not None:
            self.__setitem__('ddl_time', value)
    ...
    def estimate_reward(self, profile_info):
        ...
        self.policy_list = allocate_set(self.__getitem__('num_gpus'))
    ...
    def get_network_load(self, job_dict):
        # TODO: refactor
        if 'num_gpus' not in job_dict or 'model' not in job_dict:
            print('No gpu/model information')
            return
        
        num_w = job_dict['num_gpus']
        num_ps = num_w
...
class JobManager(object):
    ...
    def create_single_node_placement(self, job, switch_id, node_id, num_gpus, num_cpus, mem=0):
        '''
