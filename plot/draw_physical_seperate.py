...
def init_plot(ncols, **kwargs): 
    ...
    if isinstance(ncols, tuple): 
        ...
        axes = [axes[i][j] for i in range(ncols[0]) for j in range(ncols[1])]
...
def main_jct_bar(info_by_method, save_path, ax, fig, **new_template):
    ...
    if True: 
        ...
        if template.get('yname', None) is not None: 
...
if __name__ == '__main__': 
    ...
    fig, axes = init_plot((2, 4), grid=True)
    ...
    for model_id, model_name in enumerate(model_name_list):
        ...
        for (schedule, sched_verbose) in [('titan', '-False-True'), ('pollux', ''),  ('optimus', ''), ('tiresias', '')]: # , ('pollux', '')]: 
            jct_list = list() 
            makespan_list = list() 
        ...
        main_jct_bar(info_by_method=jct_info_by_method, save_path='plot/images/{}.jpg'.format(model_name), ax=axes[model_id], fig=fig,  **new_template)
        ...
        main_makespan_bar(info_by_method=makespan_info_by_method, save_path='plot/images/{}.jpg'.format(model_name), ax=axes[model_id + len(model_name_list)], fig=fig,  **new_template)
