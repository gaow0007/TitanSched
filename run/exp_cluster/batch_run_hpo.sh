...
do 
    ...
    do  
        if [[ "$trace" == *"$match"*  ]]; then 
            ...
            num_gpu_p_node=4
            add_ckpt=30
            for temporal_transferability in True False # False  # True False 
            do 
                ...
                do 
                    ...
                    ident="${schedule}_${trace}_temporal_transfer_${temporal_transferability}"
                    save_log_dir=result/$schedule/$trace-${temporal_transferability}
                    # extra_cmd=""
