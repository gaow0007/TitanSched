...
def max_reward_policy(job_list, all_gpu_list, all_resource_list, logger):
    # print([job.required_gpu_num for job in job_list])
    # print(all_gpu_list)
    # unjobs = [job for job in job_list if job.required_gpu_num == 6]
    # unjob = unjobs[0]
    ...
    logger.info('show reward ')
    logger.info('gpu_list {}'.format(all_gpu_list))
    for idx, job in enumerate(job_list):
        logger.info('{}'.format(job.required_gpu_num))
    logger.info('reward diff -- {}'.format(maxval - minval))

    return min([reward_policy1, reward_policy2, reward_policy3, reward_policy4], key=lambda x: x[0])
...
class LocalSearchRevPlaceMent(BasePlaceMent):
    __alias__ = 'local_search_rev'
    def __init__(self, cluster, name, model_info):
        super(LocalSearchRevPlaceMent, self).__init__(cluster=cluster, name=name, model_info=model_info)
