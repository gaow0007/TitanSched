...
from .chronus_solver import NoPreemptMIPSolver, compute_maximum_lease
...
class TetriSchedScheduler(BaseScheduler):
    def __init__(self, job_manager, cluster_manager, user_manager, placement, name, logger, **kwargs):
        super(TetriSchedScheduler, self).__init__(job_manager, cluster_manager, user_manager, placement=placement, name=name, logger=logger)
        assert self.name == 'tetri-sched'
        ...
        # mip_solver
        self.mip_solver = NoPreemptMIPSolver('greedy')
    ...
    def flush_lease_jobs(self, prev_time, cur_time): 
        ...
        if len(required_lease_list) > 0 and self.cluster_manager.check_free_gpus() > 0:
            # if len(in_block_list) > 20: 
            #     for job in in_block_list: 
            #         print('name {}, best effort {}, ddl_time {}, progress {}, cur_time {}, placement {}, delta {}'.format(job.name, \
            #             self.is_best_effort(job), job.ddl_time, job.application.max_progress, cur_time, job.placement, job.ddl_time - job.application.max_progress - cur_time))
            ...
            for idx, job in enumerate(in_block_list):
                ...
                if solution_matrix[idx] is not None and solution_matrix[idx][0] == 1: 
