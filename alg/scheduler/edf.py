...
class EDFScheduler(BaseScheduler):
    def __init__(self, job_manager, cluster_manager, user_manager, placement, name, logger, **kwargs):
        super(EDFScheduler, self).__init__(job_manager, cluster_manager, user_manager, placement=placement, name=name, logger=logger)
        assert self.name == 'edf'
    ...
    def miss_ddl_job(self, job, cur_time):
        return (not self.is_best_effort(job) and cur_time > job.ddl_time)
    ...
    def flush_runnable_jobs(self, prev_time, cur_time): 
        ...
        self.runnable_jobs.sort(key=lambda e: (self.is_best_effort(e), e.ddl_time, e.submission_time)) 
